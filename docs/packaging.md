...
Package version in-tree is **1.2.0** (`lakefile.lean`, `Grpc.version`). Hosted docs: [rileybetts.ai/oss/lean-grpc](https://rileybetts.ai/oss/lean-grpc). Creating the git tag on origin is a separate maintainer step (below).
...
5. **Manual version tagging (maintainer only):** on `main`, when you choose to publish:
   ...
   ```bash
   git tag -a v1.2.0 -m "lean-grpc 1.2.0"
