...
Package version: **1.1.0** (`lakefile.lean` / `Grpc.version`).
...
After [Reservoir](https://reservoir.lean-lang.org/) indexes the package you can use `require «lean-grpc»` without a git URL. Pin a commit SHA if the `v1.1.0` tag is not yet on the remote you use.
